...
tasks.register<Copy>("CopyOutputToModsFolder") {

    doFirst { //Clean build artifacts from Mods folder
        delete(fileTree(gradle.extra["targetDir"]!!) {
            include(project.name + "*.jar");
        })
    }

    from(layout.buildDirectory.dir("libs"));
    into(gradle.extra["targetDir"]!!);
    exclude("*-sources.jar");
}

tasks.named("build") {
    finalizedBy("CopyOutputToModsFolder");
}
